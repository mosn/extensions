...
func TestBumsReq2CdReq_GetXmlBytes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			buf := buffer.NewIoBufferString(tt.fields.value)
			br2cd, err := NewBums2Cd(context.TODO(), tt.fields.header, buf, &Config{})
			assert.NoError(t, err)
			got, err := br2cd.GetXmlBytes(&relation)
...
func TestBumsReq2CdReq_BodyHead(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cd.CreateProcInst("xml", `version="1.0" encoding="UTF-8"`)
			element := cd.CreateElement("service")
			err = br2cd.BodyHead(element, &relation)
			assert.Equal(t, err, tt.wantErr)
