...
func TestCd2Bums_HeadInBody(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c2b, err := NewCd2Bums(context.TODO(), tt.fields.header, tt.fields.buf, &Config{})
			assert.NoError(t, err)
			got, err := c2b.HeadInBody(&relation)
...
func TestCd2Bums_BodyInBody(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c2b, err := NewCd2Bums(context.TODO(), tt.fields.header, tt.fields.buf, &Config{})
			assert.NoError(t, err)
			got, err := c2b.BodyInBody(&relation)
			assert.Equal(t, err, tt.wantErr)
			assert.Len(t, got.String(), len(tt.want))
		})
	}
