...
func NewBums2Cd(header api.HeaderMap, value string, config *Relation) (*Bums2Cd, error) {
	bm2cd := &Bums2Cd{
		config: config,
		header: header,
	}
...
func (bm2cd *Bums2Cd) Head() (api.HeaderMap, error) {
	// TODO
	return nil, nil
}

func (bm2cd *Bums2Cd) Transcoder() (api.HeaderMap, api.IoBuffer, error) {
	header, err := bm2cd.Head()
	if err != nil {
		return nil, nil, err
	}

	body, err := bm2cd.GetXmlBytes()
	if err != nil {
		return nil, nil, err
	}
...
func (bm2cd *Bums2Cd) GetXmlBytes() ([]byte, error) {
	cd := etree.NewDocument()
	cd.CreateProcInst("xml", `version="1.0" encoding="UTF-8"`)
	element := cd.CreateElement("service")
	if err := bm2cd.BodyHead(element); err != nil {
		return nil, err
	}
	if err := bm2cd.Body(element); err != nil {
		return nil, err
	}
	return cd.WriteToBytes()
}

func (bm2cd *Bums2Cd) BodyHead(root *etree.Element) error {
	sysHead := bm2cd.genHeadElement(root, "sys-header", "SYS_HEAD")
	bm2cd.writeHead(sysHead, bm2cd.config.SysHead, bm2cd.header, bm2cd.head)
	appHead := bm2cd.genHeadElement(root, "app-header", "APP_HEAD")
	bm2cd.writeHead(appHead, bm2cd.config.AppHead, bm2cd.header, bm2cd.head)
	localHead := bm2cd.genHeadElement(root, "local-header", "LOCAL_HEAD")
	bm2cd.writeHead(localHead, bm2cd.config.LocalHead, bm2cd.header, bm2cd.head)
	return nil
}
...
func (bm2cd *Bums2Cd) Body(root *etree.Element) error {
	body := root.CreateElement("body")
	bm2cd.body.Visit(bm2cd.visitBody(body, bm2cd.config.Body))
